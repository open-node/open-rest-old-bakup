...
var utils = {
  ...
  findAllOpts: function(params, resource, association, method) {
    return _.extend({
        where: utils.where(params, resource, association, method),
        order: utils.sort(params, method.sort)
      }
  ...
  where: function(params, resource, association, method) {
    var foreignKey
      , where = {};
    _.each(method.filter, function(x) {
      if(params[x]) {
        where[x] = params[x]
      }
    });

    // 根据资源关系，自动添加父级资源的id限定条件
    if(association) {
  ...
  sort: function(params, conf) {
    ...
    if(params.sort) {
      ...
      if(!conf.allow || conf.allow.indexOf(order) < 0) {
        order = null;
      } else {
        order = [order, by]
      }
