...
_.extend(Rest.prototype, {
  ...
  list: function(Model, resource, association) {
    var recordTotalHeaderName = this.opts.recordTotalHeaderName
      , handleList            = this.handleList(Model, resource, association);
    return function(req, res, next) {
      ...
      Model.count({where: options.where}).success(function(total) {
        Model.findAll(options).success(function(list) {
          ...
          res.send(200, handleList(req, list));
          next();
        });
      });
    };
  ...
  modify: function(Model, resource, association) {
    ...
    return function(req, res, next) {
      var errors;
      attr = utils.pickParams(req, resource);
      delete attr.id
  ...
  add: function(Model, resource, association) {
    ...
    return function(req, res, next) {
      var model
        , errors
        , attrs = utils.pickParams(req, resource);
  ...
  // 返回handleList 钩子函数，只返回一个，内部多个钩子会依次顺序执行
  handleList: function(Model, resource, association) {
    var fns = this.opts.hooks.handleList
      , opts = {Model: Model, resource: resource, association: association};
    return function(req, list) {
      _.each(fns, function(fn) {
        list = fn.action(opts, req, list);
      });
      return list;
    };
  }
